...
function TambahPelanggan({ token }: { token: string }) {
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<CreatePelangganValues>({
    resolver: zodResolver(createPelangganSchema),
  });

  const mutation = useMutation({
    mutationFn: async (values: CreatePelangganValues) => {
      return await createPelanggan(values, token);
    },
    onSuccess: (response) => {
  ...
  const onSubmit = (data: CreatePelangganValues) => {
