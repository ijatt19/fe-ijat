...
function DeleteMesin({ data, token }: { data: Mesin; token: string }) {
  ...
  const getStatusBadge = (status: StatusMesin) => {
    ...
    return (
      <span className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium capitalize ${config.color}`}>
        {config.label}
      </span>
    );
  };
      ...
      <DialogContent className="sm:max-w-md">
            ...
            <div className="flex-1 max-w-sm overflow-hidden">
              ...
              <div className="mt-2 flex flex-wrap gap-2 text-xs">
                <div className="flex items-center gap-1 bg-blue-50 rounded px-2 py-1">
                  <span className="text-blue-700">{data.merk}</span>
                </div>
        ...
        <div className="flex items-start gap-2 p-3 bg-red-50 rounded-lg border border-red-100 max-w-sm">
          ...
          <p className="text-sm text-red-700 overflow-hidden">
            Data mesin <span className="font-semibold">{data.nama}</span> akan dihapus secara permanen.
          </p>
        </div>
